...
/****************************************************************************/
// ----------------- PROJECT's stuff starts from here on -----------------------
...
void rename_(char *oldname, char *newname) 
{
	...
	for (i=0; i<3; i++) 
	{
		...
		for (j=0; j<4; j++) 
		{
			...
			if(strncmp(_directory_entries[j].fname, oldname, strlen(oldname)) == 0)
			{
				// file or directory found, change name and exit!
				strncpy(_directory_entries[j].fname, newname, 252);

				writeFS304(blocks[i],(char *)_directory_entries);	// now write this block back to the disk
...
int count_differences(char *s1, char *s2)
{
    int len1;
    int len2;
    int diff;
    ...
    /* if strings are equal return 0 differences */
    if (strcmp (s1, s2) == 0) {
        return 0; /* or s2 */
    }
...
void compare(char *filename1, char *filename2)
{
	...
	char *content_f1 = (char*)malloc(3*1024*sizeof(char));
	char *content_f2 = (char*)malloc(3*1024*sizeof(char));

	// read inode entry for current directory
	// in FileSystem, an inode can point to three blocks at the most
	itype = _inode_table[CD_INODE_ENTRY].TT[0];
	blocks[0] = stoi(_inode_table[CD_INODE_ENTRY].XX,2);
	blocks[1] = stoi(_inode_table[CD_INODE_ENTRY].YY,2);
	blocks[2] = stoi(_inode_table[CD_INODE_ENTRY].ZZ,2);
	...
	for(f=0; f<2; f++)
	{
		...
		int found_file = 0;		// flag for ERROR: no file found with name %s
		char *fname;
		if(f==0)
			fname = filename1;
		else
			...
			{ 
				...
				if(strncmp(_directory_entries[j].fname, fname, strlen(fname)) == 0)
				{
					...
					if(_inode_table[e_inode].XX[0]!='0' || _inode_table[e_inode].XX[1]!='0')
					{
						...
						readFS304(blockNumber, content);
	
						if(f==0)
							strcpy(content_f1, content);
						else
							strcpy(content_f2, content);
					...
					if(_inode_table[e_inode].YY[0]!='0' || _inode_table[e_inode].YY[1]!='0')
					{
						...
						readFS304(blockNumber, content);
						if(f==0)
							strcat(content_f1, content);
						else
							strcat(content_f2, content);
					...
					//contentF1 = concat(2,content1,content2);

					//snprintf(contentF1, sizeof(contentF1), "%s%s%s", content1, content2, content3);
	...
	// --- FOR DEBUGGING ---
	//printf("File1's content:\n %s\n", content_f1);
	//printf("File2's content:\n %s\n", content_f2);

	printf("Differences in files:%d\n",count_differences(content_f1,content_f2));
...
void copy(char *filename1, char *filename2)
{
	...
	// ---- confirm that the file (filename1) to copy exists!
	char content1[1024] = "";
	char content2[1024] = "";
	char content3[1024] = "";
	//char *inputBuffer = (char*)malloc(3*1024*sizeof(char));
	...
	for (i=0; i<3; i++) 
	{
		...
		for (j=0; j<4; j++) 
		{
			...
			{ 
				...
				if(strncmp(_directory_entries[j].fname, filename1, strlen(filename1)) == 0)
				{
					found_file = 1;
					// We've found our file.
					// Now we can display its content.
					if(_inode_table[e_inode].XX[0]!='0' || _inode_table[e_inode].XX[1]!='0')
					{
						...
						readFS304(blockNumber, content1);
						//puts(content1);		// print contents for Debugging

						lenstr += strlen(content1);
					}
					if(_inode_table[e_inode].YY[0]!='0' || _inode_table[e_inode].YY[1]!='0')
					{
						// There is data on this block. Let's read it.
						...
						readFS304(blockNumber, content2);
						//puts(content2);		// print contents for Debugging

						lenstr += strlen(content2);
					}
					if(_inode_table[e_inode].ZZ[0]!='0' || _inode_table[e_inode].ZZ[1]!='0')
					{
						// There is data on this block. Let's read it.
						...
						readFS304(blockNumber, content3);
						//puts(content3);		// print contents for Debugging

						lenstr += strlen(content3);
					}
	...
	for (i=0; i<3; i++) 
	{
		...
		for (j=0; j<4; j++) {
			...
			if (strncmp(filename2,_directory_entries[j].fname, 252) == 0) { // compare with user given name
					printf("%.252s: Already exists.\n",filename2);
					return;
			}
		}
	...
	{ 
		...
		readFS304(blocks[empty_dblock],(char *)_directory_entries);		// read block of current directory where info on this new directory will be written
		_directory_entries[empty_dentry].F = '1';					// remember we found which directory entry is unused; well, set it to used now
		strncpy(_directory_entries[empty_dentry].fname,filename2,252);	// put the name in there
		itos(_directory_entries[empty_dentry].MMM,empty_ientry,3);		// and the index of the inode that will hold info inside this directory
		writeFS304(blocks[empty_dblock],(char *)_directory_entries);	// now write this block back to the disk
		...
		if(lenstr<1024)
		{
			writeFS304(newblock1,content1);
			itos(_inode_table[empty_ientry].XX, newblock1,2);
			// set the rest of two inodes to empty
			strncpy(_inode_table[empty_ientry].YY,"00",2);
			strncpy(_inode_table[empty_ientry].ZZ,"00",2);
		}
		else if (lenstr<2048)
		...
		else
		{
			...
			writeFS304(newblock3,content3);			
		}

		
		// create the inode entry...first, its a file, so FI

		writeFS304(BLOCK_INODE_TABLE, (char *)_inode_table);	//  write the inode table back to the disk
